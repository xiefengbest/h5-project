...
```
pages-mobile/                   ← 手机 App 原型（390px）
├── wallpaper/
├── minus-one/
└── web-anyselected/

pages-pad/                      ← Pad App 原型（1280px，与 pages-mobile/ 平级）
├── wallpaper/
├── minus-one/
└── web-anyselected/
```

| 模块 | 手机（`pages-mobile/`） | Pad（`pages-pad/`） |
|------|------------------|---------------------|
...
```bash
open pages-mobile/wallpaper/index.html
open pages-mobile/minus-one/news.html
open pages-mobile/minus-one/discover.html
open pages-mobile/web-anyselected/index.html
open pages-mobile/wallpaper/short-dramas.html
```
...
若需打包发给他人，可运行（结构与 `pages-mobile/` 完全一致，链接无需改写）：

```bash
bash scripts/deploy.sh
```

日常开发直接改 `pages-mobile/` 即可，`dist/` 可随时重新生成。
